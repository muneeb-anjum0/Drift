...
def check_python_dependencies() -> None:
    ...
    try:
        ...
        if major >= 5:
            problems.append(
                f"transformers {transformers_version} is installed, but the merge workflow expects transformers 4.x"
            )
    except Exception:
        pass
    if problems:
        raise SystemExit(
            "Q3_K_M build dependencies are not ready: "
            + "; ".join(problems)
...
def run(script: str, force: bool = False) -> None:
    ...
    if force and script.endswith("build_q3km_model.py"):
        cmd.append("--force")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build DriftLedger Q3_K_M GGUF from the existing LoRA adapter.")
    ...
    if not args.force and nonempty_file(gguf_q3km_path(root)):
        print(f"Q3_K_M GGUF already exists: {gguf_q3km_path(root)}", flush=True)
        return

    if args.force or not merged_model_valid(merged_model_dir(root)):
    ...
    if args.force or not nonempty_file(gguf_f16_path(root)):
        run("tools/convert_merged_to_gguf.py")
    else:
        print("Skipping F16 GGUF conversion; output already exists.", flush=True)

    if args.force or not nonempty_file(gguf_q3km_path(root)):
        run("tools/quantize_gguf_q3km.py")
    else:
        print("Skipping Q3_K_M quantization; output already exists.", flush=True)

    if not gguf_q3km_path(root).exists() or gguf_q3km_path(root).stat().st_size == 0:
        raise SystemExit("Q3_K_M GGUF was not created.")
    print(f"Q3_K_M GGUF ready: {gguf_q3km_path(root)}", flush=True)
